"""
The following contains the implementation of the MMD metrics
The implementation is adapted from https://github.com/cvignac/DiGress
"""
...
###############################################################################
#
# Adapted from https://github.com/lrjconan/GRAN/
# which in turn is adapted from https://github.com/JiaxuanYou/graph-generation
#
###############################################################################
...
from src.metrics.dist_helper import (
    compute_mmd,
    emd,
    gaussian,
    gaussian_emd,
...
class SpectreSamplingMetrics(nn.Module):
    ...
    def forward(self, generated_graphs: list):
        print(
            f"Computing sampling metrics between {len(generated_graphs)} "
            f"generated graphs and {len(self.test_graphs)}"
            f" test graphs -- emd computation: {self.compute_emd}"
        )
        networkx_graphs = []
        # adjacency_matrices = []
        ...
        table_log = {k: [v] for k, v in to_log.items()}
        print(tabulate(table_log, headers="keys", tablefmt="grid", floatfmt=".5f"))

    def reset(self):
        pass


class CommunityDatasetSamplingMetrics(SpectreSamplingMetrics):
    def __init__(self, dataloaders: dict[str, DataLoader]):
        super().__init__(
            dataloaders=dataloaders,
            compute_emd=False,  # changed to False as it takes too long to compute
